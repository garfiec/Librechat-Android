...
- **v0.8.8-line partial sync (untagged dev commit `6c97a7f4`, 2026-07-23):** three NEW `supportsFeature` date
  gates — the `context-projection` POST suppression, mid-run steering, and the tool-favorites probe — plus one
  deliberate NON-gate, the human-in-the-loop pause surfaces (rows above). Both real gates are date gates for the same reason. The target is untagged: upstream removed `POST /api/endpoints/context-projection` in #13953 (landing commit
