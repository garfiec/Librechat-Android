...
- `stopGeneration()` POSTs `chatRepository.abortChat()` and **deliberately does not cancel the stream job.** `POST /api/agents/chat/abort` only acks (`{ success, aborted }`); the server ends the run by emitting an ordinary `final` frame flagged `aborted` over the same SSE stream, and *that* frame carries the partial's content parts. Cancelling the collector was the original bug — it discarded the frame, leaving nothing to render and forcing a refetch that raced the server's async persistence (the request message is saved only on `request.js`'s unwind), which is why a stopped reply vanished and "came back" only after enough time had passed. `handleFinal` reads `aborted` off the frame, not off local stop state, so a Stop issued from another client is handled identically. Stop-specific behavior: no auto-TTS, no `gen_title`, no queue drain (the hold is re-asserted). `abortRequested` guards double-taps — `isStreaming` can't, since the stream stays live until the frame lands. If the abort POST itself fails no frame is coming, so `forceStopStream` ends it locally (partial preserved, queue held, conversation re-read)
- **Aborted-final caching (mobile-only concern).** The web client has no local store; we do, and the aborted frame is deliberately poorer than a completed one — skeletal `requestMessage`, no `text`, a stub `conversation`, a hardcoded `New Chat` title. So it is normalized at one chokepoint (`normalizeAbortedFrame`) before anything renders or caches it: the skeletal request is swapped for the richer in-memory copy (`preferInMemoryRequest` — `mergeFinalMessagesInMemory` replaces wholesale by id, so without this the user message loses its attachments on screen *and* on disk), and `text` is rebuilt from the content parts to match the row the server saved with `parseTextParts`. Two writes are then suppressed: `saveConversation` (the stub would overwrite the real title/model — `upsertPreservingTags` carries forward only tags) and `cacheTurn` when `abortWasPersistedServerSide()` is false. That last one matters because the server saves the stopped response only if it has persistable content, while still sending a non-null `responseMessage` — cache a contentless one and you get a blank bubble that survives every reload, since `getMessages` upserts and never deletes rows the server didn't return. Title is re-read network-first rather than generated: with the default `immediate` timing a title that finished before the Stop is kept server-side, and the cached row still holds the `New Chat` placeholder a cache-first read would return
